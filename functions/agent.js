...
async function runAgent(env, { event, prefs, profile, me }) {
  me = me || DEFAULT_ME;
  ...
  // candidate pool: attendees with a headline (skip yourself)
  const candidates = await cypher(
    env,
    `MATCH (:Event {id:$event})<-[:ATTENDS]-(p:Person)
     WHERE p.headline IS NOT NULL AND p.headline <> '' AND p.id <> $me
     ...
     ORDER BY events DESC LIMIT 150`,
    { event, me },
  );
  ...
  if (!picks.length) picks = candidates.slice(0, 6).map((_, i) => ({ idx: i, why: 'Well-connected across the scene', opener: '' }));
  ...
  return { event: { id: event, name: evRow.name }, totalAttendees: total, shortlist: shortlist.filter(Boolean) };
}
    ...
    try {
      ...
      if (uid && ctx?.db) {
        const vals = [
          ...
          params.profile || '', params.me || null, result.totalAttendees,
          JSON.stringify(result.shortlist || []),
        ];
        const { rows } = await ctx.db.query(
          'SELECT id FROM runs WHERE user_id = $1 AND event_id = $2',
          [uid, result.event.id],
        );
        if (rows[0]) {
          await ctx.db.query(
